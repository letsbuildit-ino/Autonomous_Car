...
void loop(){
  ...
  else if (x==0 && y==1 && z==0){  /// extreme right
    cflag=6;
  ...
  else if (x==1 && y==0 && z==0){  /// hold
    hold();
  }
  else if (x==0 && y==0 && z==0){  /// return back
    if(k==0){
      hold();
    }else{
      lastcommand();
...
void returnback(){
  ...
  for (int p=k; p<k+1 && p>-1; p--){
    Serial.println("");
    Serial.println("im here in return loop");
